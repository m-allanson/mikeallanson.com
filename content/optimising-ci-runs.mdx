...
Meant that `longJob` would only run after all the `shortJob`s failed. This made individual CI runs take slightly longer, but massively reduced the overall number of CI minutes used.
...
Remove, reduce or re-use. I guess that's a general lesson that can be applied to any code.
